...
def dict_to_object(d):
    """Рекурсивно преобразует словарь в объект"""
...
def frame_segment_audio(dataset, config_path, output_dir = "frame_vad_outputs" ):
    """
    This function peforms VAD on each 20ms frames of the input audio files. 
    ...
    if not os.path.exists(cfg.frame_out_dir):
        ...
        try:
            shutil.rmtree(cfg.frame_out_dir)
        except OSError as err:
            logging.exception(err)
        os.mkdir(cfg.frame_out_dir)
        logging.info(f"Frame predictions already exist at {cfg.frame_out_dir}, contents will be removed.")
        extract_frame_preds = True
    ...
    rttm_out_dir = generate_vad_segment_table(
        vad_pred_dir=pred_dir,
        postprocessing_params=cfg.vad.parameters.postprocessing.__dict__,
        frame_length_in_sec=frame_length_in_sec,
        num_workers=cfg.num_workers,
...
class SegmentDataset(Dataset):
    """
    Dataset that streams arbitrary (offset, duration) windows
    from a single large wav file.
    offset, duration are in *seconds*.
    """
    def __init__(self, wav_path: str, segments):
        """
        segments : iterable of dicts:
                   {"offset": float(sec), "duration": float(sec)}
        """
        self.wav_path = wav_path
        self.segments = list(segments)

        # open once to obtain sample rate
        with sf.SoundFile(self.wav_path) as f:
            self.sr = f.samplerate
    ...
    def __getitem__(self, idx):
        ...
        # read exactly the requested slice
        frames = int(np.round(dur * self.sr))
        start_frame = int(np.round(offset * self.sr))

        with sf.SoundFile(self.wav_path) as f:
            f.seek(start_frame)
            audio = f.read(frames=frames, dtype='float32', always_2d=False)

        print(f"[SegmentDataset] offset={offset:.2f}s  duration={dur:.2f}s  "f"samples={len(audio)}  ({len(audio)/self.sr:.2f}s)")
        # mono-ise
        if audio.ndim == 2:
            audio = audio.mean(axis=1)

        return torch.from_numpy(audio), seg  # returns (tensor, meta)
